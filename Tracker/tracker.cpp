...
void read_db_torrents()
{
  ...
  try
  {
    ...
    for (auto row : query("select info_hash, @fid, down_multi, up_multi from @torrents where flags != 0 and flags & 2"))
    {
      if (torrent_t* t = find_ptr(g_torrents, to_array<char, 20>(row[0])))
      {
        t->down_multi = row[2].f();
        t->up_multi = row[3].f();
      }
      query("update @files set flags = flags & ~2 where @fid = ?", row[1].i());
    }
    if (g_config.auto_register_ && !g_torrents.empty())
      return;
    for (auto row : query("select info_hash, @completed, @tid, ctime, down_multi, up_multi from @torrents where @tid >= ?", g_tid_end))
    {
...
void write_db_users()
{
  ...
  if (!g_torrents_users_updates_buffer.empty())
  {
    ...
    async_query("insert ignore into @torrents_users (active, completed, downloaded, `left`, uploaded, completed_time, down_rate, up_rate, peer_id, ipa, port, mtime, fid, uid) values ?"
      " on duplicate key update"
...
string srv_insert_peer(const tracker_input_t& in, bool udp, user_t* user)
{
  ...
  if (user && t.tid)
  {
      ...
      && in.uploaded_ >= p->uploaded)
    {
      ...
      time_t t = std::max<time_t>(1, srv_time() - p->mtime);
    ...
    g_torrents_users_updates_buffer += make_query(g_database, "(?,?,?,?,?,?,?,?,?,?,?,?,?,?),",
      ...
      in.left_,
      uploaded,
      in.event_ == tracker_input_t::e_completed ? srv_time() : -1,
      down_rate,
      up_rate,
      in.peer_id_,
      ntohl(*reinterpret_cast<const uint32_t*>(&in.ipv6_[12])),
      ntohs(in.port_),
      srv_time(),
      t.tid,
      user->uid);
    if (downloaded || uploaded)
      g_users_updates_buffer += make_query(g_database, "(?,?,?),", static_cast<long long>(downloaded * t.down_multi / 100), static_cast<long long>(uploaded * t.up_multi / 100), user->uid);
